...
def TU_assembly(tu_name):
    ...
    for path in [promoter_vector_path, gene_vector_path, terminator_vector_path]:
        if path.exists():
    ...
    if promoter_vector_path.exists():
        files[promoter_vector_path] = promoter_vector_path.read_text()
    else:
        files[promoter_vector_path] = f"{promoter_vector_path.name} is missing."

    if gene_vector_path.exists():
        files[gene_vector_path] = gene_vector_path.read_text()
    else:
        files[gene_vector_path] = f"{gene_vector_path.name} is missing."

    if terminator_vector_path.exists():
        files[terminator_vector_path] = terminator_vector_path.read_text()
    else:
        files[terminator_vector_path] = f"{terminator_vector_path.name} is missing."
 
    assert len(files[promoter_vector_path]) > 1
    assert len(files[gene_vector_path]) > 1
    assert len(files[terminator_vector_path]) > 1
    
    assert cloned(pYPKa, ZraI,  prom)
    assert cloned(pYPKa, AjiI,  gene)
    assert cloned(pYPKa, EcoRV, term)
        
    files[promoter_vector_path] = read(files[promoter_vector_path])
    files[gene_vector_path] = read(files[gene_vector_path])        
    files[terminator_vector_path] = read(files[terminator_vector_path])
    
    assert files[promoter_vector_path].circular
    assert files[gene_vector_path].circular
    assert files[terminator_vector_path].circular
